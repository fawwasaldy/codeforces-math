...
void solve() {
    ...
    sort(all(num));
    fo(i,n){
        ll a = upper_bound(num.begin(), num.end(), r-num[i]) - num.begin();
        ll b = lower_bound(num.begin(), num.end(), l-num[i]) - num.begin();
        res += a;
        res -= b;
        deb2(a, b);
        if (l <= 2ll*num[i] && 2ll*num[i] <= r) {
            res--;
        }
    }
    cout << res/2ll << endl; 
